...
function replacePlayer(oldPlayer, newPlayer, gameId) {
  ...
  console.log('Démarrage d\'une nouvelle partie entre:', firstPlayer.playerName, '(X) et', secondPlayer.playerName, '(O)');
...
wss.on('connection', function connection(ws) {
  ws.on('message', async function incoming(message) {
    ...
    if (data.type === 'move' && ws.opponent) {
      ...
      if (!game) {
        console.log('Partie non trouvée pour l\'ID:', ws.gameId, 'Joueur:', ws.playerName);
      ...
      const isFirstPlayer = (game.player1GoesFirst && game.player1 === ws) || 
      ...
      console.log('Coup valide par', ws.playerName, ':', data.index, 'avec le symbole', mark);
      ...
      if (winner || game.board.every(cell => cell !== null)) {
        ...
        if (winner) {
          ...
          const winningPlayer = (game.player1GoesFirst && isXWinner) || (!game.player1GoesFirst && !isXWinner)
            ? game.player1
            : game.player2;
          const losingPlayer = winningPlayer === game.player1 ? game.player2 : game.player1;
          
          console.log('Gagnant:', winningPlayer.playerName, 'Perdant:', losingPlayer.playerName);

          if (playerQueue.length === 0) {
            // Pas de joueurs en attente, demande aux deux joueurs s'ils veulent continuer
            console.log('Pas de joueurs en attente - demande aux deux joueurs s\'ils veulent continuer');
          ...
          if (playerQueue.length === 0) {
            ...
            console.log('Joueurs en attente - Démarrage d\'une nouvelle partie avec les mêmes joueurs');
            ...
            const firstPlayer = game.player1GoesFirst ? game.player1 : game.player2;
            const secondPlayer = game.player1GoesFirst ? game.player2 : game.player1;
    ...
    if (data.type === 'continue_choice') {
      ...
      console.log('Choix de continuation de', ws.playerName, ':', data.continue);
      ...
      if (game.drawResponses) {
        ...
        if (game.drawResponses.player1Response !== null && 
            game.drawResponses.player2Response !== null) {
          
          if (game.drawResponses.player1Response && game.drawResponses.player2Response) {
            // Les deux joueurs veulent continuer
            console.log('Les deux joueurs ont choisi de continuer après le match nul');
            ...
            console.log('Au moins un joueur a choisi de ne pas continuer après le match nul');
      ...
      if (game.winResponses) {
        ...
        // Vérifie si on a les deux réponses
        if (game.winResponses.winnerResponse !== null && 
            game.winResponses.loserResponse !== null) {
          
          if (game.winResponses.winnerResponse && game.winResponses.loserResponse) {
            // Les deux joueurs veulent continuer
            console.log('Les deux joueurs ont choisi de continuer après la victoire');
            ...
            // Au moins un joueur veut partir
            console.log('Au moins un joueur a choisi de ne pas continuer après la victoire');
      ...
      if (data.continue) {
        if (playerQueue.length > 0) {
          ...
          console.log('Taille de la file d\'attente après remplacement:', playerQueue.length);
